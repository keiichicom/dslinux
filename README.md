...
his is a fork of the original dslinux hosted here: https://www.dslinux.org/, which now runs on the Nintendo DSi and DSi XL. Note that if you are a Nintendo ds/ds lite owner, you should still use the original dslinux.

The pre-release includes 1 game with dldi disabled. The reason for it being disabled is that the dldi driver that comes with the original dslinux is only for flashcarts. So to get it working, I would either have to create a new dldi driver or a Linux kernel mmc/sd host driver which is not done yet. As a result of this there can be no Linux directory with games on the sdcard since Linux can't read it. So I installed one game to the ROMFS instead which is advent4 (Colossal Cave Adventure) for proof-of-concept purposes. An improvement over the original dslinux is that I could specify the available 16mb of RAM that is on the DSi / DSi XL, since the memory expansion pack (required by the older dslinux) can't be installed on the DSi / DSi XL since there is no GBA slot on those devices.
...
Note: I am having trouble with the github source tree not matching my local, so the build may not work. If that is the case for you take this archive: dslinux-backup.tar.gz and unpack the contents into dslinux (without the top folder: dslinux-backup. You need git lfs (large file storage) installed to download it.
