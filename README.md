...
Then unzip the file: dslinux-dldi.tgz. Then in the linux/etc folder, copy rc.defaults to rc.conf and change the essid from "ram" to your ESSID. Then setup hostapd on a computer you own with WiFi, with the same ESSID, that supports AP mode (open security) and also setup DNS masquerade and MAC address filtering (google for instructions). Then put the linux folder in the root of your slot 1 flashcart SD card (Some flashcarts will auto-patch the DLDI driver in dslinux.nds and some won't. Search the web if you need a patch(.dldi file) for your flashcart and patch dslinux.nds with dlditool(download it)). Then put the dslinux.nds file in your roms/nds or games/nds folder. When you start dslinux.nds on your nds or ds lite WiFi should bre configured(ignore the error). Test it by pinging 8.8.8.8 (google dns server) and www.yahoo.ca.
