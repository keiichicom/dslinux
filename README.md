...
Then unzip the file: dslinux-dldi.tgz. Then in the linux/etc folder, copy rc.defaults to rc.conf and change the essid from "ram" to your essid. Then setup hostapd on a computer you own with wifi that supports ap mode and also setup dns masquerade and mac address filtering (google for instructions). Then put the linux folder in the root of your slot 1 flashcart sd card. Then put the dslinux.nds file in your roms/nds or games/nds folder. When you start dslinux.nds on your nds or ds lite the wifi should connect (ignore the error). Test it by pinging 8.8.8.8 (google dns server) and www.yahoo.ca 
